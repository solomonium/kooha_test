...
class DateFormatter {
  static Future<String> customDatePickerFormat(
    ...
    final DateFormat _yearMonthDayFormat = DateFormat('yMd');
    ...
    if (selectedDate != null && selectedDate != initialDate) {
      if (formatStyle == 'dd-mm-yyyy') {
        //
        return _dayMonthYearFormat.format(selectedDate).toString();
        //we would add more functionality format in future
      } else if (formatStyle == 'yMd') {
        //
        return _yearMonthDayFormat.format(selectedDate).toString();
        //
      } else {
        //
        return _dayMonthYearFormat.format(selectedDate).toString();

        ///we return the default format
      }

      ///.............first block ends here
    } else {
      //this happens if user fail to select anything
      if (formatStyle == 'dd-mm-yyyy') {
        return _dayMonthYearFormat.format(DateTime.now()).toString();
      } else {
        return _dayMonthYearFormat.format(DateTime.now()).toString();
      }
    }
  ...
  static String customDateFormatter(
      {String formatStyle = 'dd-mm-yyyy',
    ...
    final DateFormat _yearMonthDayFormat = DateFormat('yMd');
    var initialDate = DateTime.now();
    final dateFromAPI = DateTime.parse(apiDate);

    if (dateFromAPI != initialDate) {
      if (formatStyle == 'dd-mm-yyyy') {
        //
        return _dayMonthYearFormat.format(dateFromAPI).toString();
        //we would add more functionality format in future
      } else if (formatStyle == 'yMd') {
        //
        return _yearMonthDayFormat.format(dateFromAPI).toString();
        //
      } else {
        //
        return _dayMonthYearFormat.format(dateFromAPI).toString();
  ...
  static String timeStampToDate(int timeStamp) {
    // DateTime dateFromTimeStamp = DateTime.parse(timeStamp.toDate().toString());
    var dateFromTimeStamp =
        DateTime.fromMillisecondsSinceEpoch(timeStamp * 1000);
    var formatedDate =
        customDateFormatter(apiDate: dateFromTimeStamp.toString());
    return formatedDate;
  }

  static String timeStampToDateCompare(int timeStamp) {
